...
def isAdmin():
    ...
    user = (
        ...
        .join(Group_user, User.id == Group_user.userid)
...
def isTeacher():
    ...
    user = (
        ...
        .join(Group_user, User.id == Group_user.userid)
        .filter(User.username == current_user.username)
        .filter(Group_user.groupid == teacherId.id)
        .first()
    )
    if user is None:
        return False
...
def find_free_num(mask):
    ...
    while 1:
        ...
        user = User.query.filter_by(username=curUser).first()
        if user is None:
            break
    ...
    if request.method == "POST":
        ...
        if data["method"] == "delete_reports":
            for report in data["reportsDelete"]:
                ...
                cur_abs_path = os.path.abspath(os.path.curdir)
                user_folder = (
                    User.query.filter_by(username=report["user"]).first().local_folder
                )
                usr_report_path = "volume/userdata/" + user_folder + "/reports/"
...
def groups_edit():
    ...
    if request.method == "POST":
        ...
        if data["method"] == "add_group":
            group = Group.query.filter_by(groupname=data["newGroup"]).first()
            if group is None:
            ...
            for userName in data["usersDelete"]:
                ...
                group_user = Group_user.query.filter_by(
            ...
            if data["groupName"] == "admin" or data["groupName"] == "teacher":
                return render_template("errors/500.html")
            group = Group.query.filter_by(groupname=data["groupName"]).first()
            Group_user.query.filter_by(groupid=group.id).delete(
    ...
    if request.method == "POST":
        ...
        if "usersDelete" in data.keys():
            for userName in data["usersDelete"]:
                ...
                Group_user.query.filter_by(userid=usr.id).delete(
                    synchronize_session=False
                )
...
def admin():
    ...
    form.vars_names.choices = [
        (val, val + ": " + str(len(os.listdir("volume/vars/" + val))) + " шт.")
        for val in (sorted(dirlist))
    ]
    ...
    return render_template(
        "admin/register.html", title="Регистрация", form=form, arUsers=[]
    )
...
def generate_vars_page():
    ...
    if var_form.validate_on_submit():
        ...
        if var_form.preview.data or var_form.create.data:
            ...
            new_params = {
                "program": var_form.program.data.replace("\r", ""),
                "p1": var_form.p1.data,
                "p2": var_form.p2.data,
                "p3": var_form.p3.data,
                "p4": var_form.p4.data,
        ...
        if var_form.create.data and not var_form.give_var.data:
            if (var_form.var_name.data in os.listdir("volume/vars")) and (
                User.query.filter_by(var_name=var_form.var_name.data).count() != 0
            ):
                # выдаём предупреждение
                return render_template(
                    "admin/var_exists.html", var_name=var_form.var_name.data
                )
        ...
        if var_form.preview.data:
            ...
            return render_template(
                "admin/variants_generation.html",
